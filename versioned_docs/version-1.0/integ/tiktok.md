...
**Sincronização de estoque VTEX → TikTok** - busca quantidade disponível na VTEX e dispara atualização na TikTok Shop.
...
| **Timeout** | 60s |
| **Memória** | 512 MB |
| **Tipo de Invocação** | Assíncrona (Event) via SQS |

#### Responsabilidade

Orquestra a sincronização de estoque entre VTEX e TikTok Shop:
1. Busca lista de SKUs do cliente (DynamoDB ou VTEX API)
2. Consulta estoque disponível na VTEX por SKU
3. Envia para fila SQS para atualização no TikTok Shop
4. Suporta processamento individual (por SKU) ou em lote
...
sequenceDiagram
    ...
    Scheduler->>Lambda: Trigger (triggerPrepareStock)
    Lambda->>DDB: Query ssxml_tik_tok_products_seller_sku
    DDB-->>Lambda: Lista de SKUs
    
    loop Lote de 20 SKUs
        Lambda->>SQS: Envia lote para fila
    end
    
    SQS->>Lambda: Consome (triggerPepareSkuStockBatch)
    
    loop Para cada SKU
        Lambda->>VTEX: GET /api/logistics/pvt/inventory/items/[sku]
        VTEX-->>Lambda: availableQuantity
        Lambda->>SQS: Envia para fila de atualização
    end
    
    SQS->>UpdateLambda: Consome mensagem
    UpdateLambda->>UpdateLambda: Atualiza estoque TikTok
```

#### Event Types Suportados

O handler suporta **4 tipos de eventos**:

| Type | Handler | Descrição |
|------|---------|-----------|
| `triggerPrepareStock` | `PrepareStock()` | Processa **todos os SKUs** do cliente em lote |
| `triggerPepareSkuStock` | `PepareSkuStock()` | Processa **1 SKU específico** (invocação direta) |
| `triggerPepareSkuStockBatch` | `PepareSkuStockBatch()` | Processa **lote de SKUs** (via SQS) |
| `triggerCountStock` | `CountStock()` | Conta estoque (não implementado) |

#### Payload de Eventos

##### 1. triggerPrepareStock (Processamento Completo)
...
**Comportamento**:
- Busca **todos os SKUs** do cliente em `ssxml_tik_tok_products_seller_sku`
- Divide em lotes de 20 SKUs
- Envia cada lote para fila SQS `SQS_BUSCA_ESTOQUE`

##### 2. triggerPepareSkuStock (SKU Individual)
...
**Comportamento**:
- Busca estoque de **1 SKU** na VTEX
- Invoca diretamente `integracao-tik-tok-ESTOQUE-TTK-atualiza-estoque-sku`

##### 3. triggerPepareSkuStockBatch (Lote de SKUs)
...
**Comportamento**:
- Busca estoque de cada SKU do lote na VTEX
- Envia para fila SQS `SQS_ATUALIZA_ESTOQUE`

#### Funções Principais

##### 1. prepareStock()

**Propósito**: Processa todos os SKUs do cliente em lote
...
async function prepareStock(clientHash) {
  ...
  // Busca todos os SKUs do cliente no DynamoDB
  const skuList = await getSkuListVtex(clientHash);
  
  // Divide em lotes de 20 e envia para SQS
  const batchSize = 20;
...
**Uso**: Sincronização completa de estoque (agendada periodicamente)

##### 2. getSkuListVtex()

**Propósito**: Busca lista de SKUs do cliente no DynamoDB
...
async function getSkuListVtex(clientHash) {
  const params = {
    ...
    ExpressionAttributeValues: { ':clientHash': clientHash }
  };
...
:::tip Por que buscar do DynamoDB?
Buscar SKUs do DynamoDB é mais eficiente que consultar todos os produtos da VTEX, pois nem todos os produtos VTEX são enviados para TikTok. Isso evita desperdício de recursos.
:::

##### 3. prepareSkuStock()

**Propósito**: Processa 1 SKU específico com invocação direta
...
async function prepareSkuStock(clientHash, sku) {
  ...
  const reservedQuantity = stockData.reservedQuantity;
  const availableQuantity = stockData.availableQuantity;
  
  console.log("availableQuantity ...", availableQuantity);
  
  // Invoca Lambda de atualização diretamente
  await dispatchStockUpdateBySku(clientHash, sku, availableQuantity);
}
```

**Uso**: Atualização em tempo real de 1 produto específico

##### 4. prepareSkuStockBatch()

**Propósito**: Processa lote de SKUs via SQS
...
**Uso**: Processamento em lote via SQS

#### API VTEX Utilizada

##### GET /api/logistics/pvt/inventory/items/\{sku\}/warehouses/\{warehouseId\}

**Propósito**: Busca estoque de um SKU em um warehouse específico
...
- `reservedQuantity`: Quantidade reservada (pedidos não enviados)
- `availableQuantity`: Disponível para venda (**usado no TikTok**)

📚 **Documentação**: [VTEX Logistics API](https://developers.vtex.com/docs/api-reference/logistics-api#get-/api/logistics/pvt/inventory/skus/-skuId-)

##### GET /api/catalog_system/pvt/sku/stockkeepingunitids

**Propósito**: Lista todos os SKUs do catálogo (alternativa, não usada por padrão)

```javascript
async function getSkuList(page = 1, pageSize = 1000) {
  const result = await axios.get(
    `${vtexCatalogUrl}/stockkeepingunitids`,
    {
      headers,
  ...
  // Recursão para paginação
  if (Array.isArray(items) && items.length > 0) {
    globalSkuList.push(...items);
    return await getSkuList(page + 1, pageSize);
  } else {
...
**Response**: `[880408, 880409, 880410, ...]`

#### Integração com SQS

##### Fila: SQS_BUSCA_ESTOQUE

**Propósito**: Envia lotes de SKUs para buscar estoque
...
##### Fila: SQS_ATUALIZA_ESTOQUE

**Propósito**: Envia SKU + quantidade para atualizar na TikTok
...
#### Invocação Direta (Lambda to Lambda)

Quando processa SKU individual, invoca diretamente a Lambda de atualização:
...
```javascript
async function setCredentials(clientHash) {
  const credentials = await getCredentials(clientHash);
  
  VTEX_ACCOUNT = credentials.vtex_account;
  VTEX_APP_KEY = credentials.vtex_app_key;
  VTEX_APP_TOKEN = credentials.vtex_token;
  
  // URLs montadas dinamicamente
  vtexInventoryUrl = `https://${VTEX_ACCOUNT}.vtexcommercestable.com.br/api/logistics/pvt/inventory`;
  vtexWarehouseUrl = `https://${VTEX_ACCOUNT}.vtexcommercestable.com.br/api/logistics/pvt/configuration/warehouses`;
  vtexCatalogUrl = `https://${VTEX_ACCOUNT}.vtexcommercestable.com.br/api/catalog_system/pvt/sku`;
  
  clientWarehouseId = credentials.vtex_warehouse_id;  // Ex: "1_1"
  clientWarehouseIdTTK = credentials.tik_tok_shop_warehouse_id;
}
```

**Credenciais Necessárias** (armazenadas no DynamoDB):
- `vtex_account`: Nome da conta VTEX
- `vtex_app_key`: App Key VTEX
- `vtex_token`: App Token VTEX
- `vtex_warehouse_id`: ID do warehouse VTEX (ex: "1_1")
- `tik_tok_shop_warehouse_id`: ID do warehouse TikTok

#### Variáveis de Ambiente

```bash
# URLs das filas SQS
SQS_BUSCA_ESTOQUE=https://sqs.us-east-1.amazonaws.com/xxx/tiktok-busca-estoque
SQS_ATUALIZA_ESTOQUE=https://sqs.us-east-1.amazonaws.com/xxx/tiktok-atualiza-estoque
...
```
🔍 Buscando lista de SKUs do cliente: 680b0418...
📊 Total de SKUs encontrados: 1.523
📦 Dividindo em lotes de 20 SKUs
📤 Enviado lote 1/77 para SQS (20 SKUs)
📤 Enviado lote 2/77 para SQS (20 SKUs)
...
✅ Todos os lotes enviados para SQS_BUSCA_ESTOQUE
```

```
🔍 Processando lote de SKUs via SQS: [880408, 880409, 880410, ...]
🔍 Buscando estoque: 880408 - 1_1
📊 stockData: { totalQuantity: 1000, reservedQuantity: 50, availableQuantity: 950 }
📤 Enviando para SQS_ATUALIZA_ESTOQUE: SKU 880408, Qty 950
✅ SQS Message Sent: abc123-def456-ghi789
```

#### Cenários de Uso

##### Cenário 1: Sincronização Completa Agendada

```javascript
// EventBridge aciona a cada X horas
const event = {
  type: "triggerPrepareStock",
  clientHash: "680b0418..."
};
...
**Fluxo**:
1. Lambda busca todos os SKUs do DynamoDB
2. Divide em lotes de 20
3. Envia cada lote para SQS_BUSCA_ESTOQUE
4. SQS consome e processa lote por lote
5. Cada SKU é enviado para SQS_ATUALIZA_ESTOQUE
6. Lambda ATUALIZA-ESTOQUE envia para TikTok

##### Cenário 2: Atualização em Tempo Real (Webhook VTEX)

```javascript
// Webhook VTEX notifica mudança de estoque
const event = {
  type: "triggerPepareSkuStock",
  clientHash: "680b0418...",
  sku: "880408"
...
**Fluxo**:
1. Lambda busca estoque do SKU na VTEX
2. Invoca diretamente Lambda ATUALIZA-ESTOQUE
3. TikTok é atualizado em poucos segundos

##### Cenário 3: Reprocessamento de Lote
...
**Fluxo**:
1. Lambda processa apenas os SKUs especificados
2. Envia para SQS_ATUALIZA_ESTOQUE

#### Monitoramento

**Métricas Recomendadas**:
- Número de SKUs processados por execução
- Tempo médio de busca por SKU na VTEX
- Taxa de sucesso de envio para SQS
- Tamanho da fila SQS_BUSCA_ESTOQUE
- Tamanho da fila SQS_ATUALIZA_ESTOQUE
- Erros de timeout da API VTEX

**CloudWatch Logs**:
```
[INFO] Total de SKUs encontrados: 1523
[INFO] Enviado lote 1/77 para SQS
[INFO] Buscando estoque: 880408 - 1_1
[INFO] availableQuantity: 950
[INFO] SQS Message Sent: abc123-def456
[ERROR] Erro ao buscar estoque SKU 880408: timeout
```

**Alarmes**:
- Taxa de erro > 5%
- Latência > 30s
- Fila SQS com > 10.000 mensagens
...
|----------|-------|---------|
| SKU não encontrado na VTEX | SKU inválido ou deletado | Remover SKU do DynamoDB |
| Warehouse ID inválido | `vtex_warehouse_id` incorreto | Verificar credenciais no DynamoDB |
| Timeout na API VTEX | VTEX lenta ou muitos requests | Aumentar timeout da Lambda |
| Fila SQS crescendo | Lambda ATUALIZA-ESTOQUE lenta | Aumentar concorrência da Lambda |
| `availableQuantity` negativo | Dados inconsistentes na VTEX | Enviar 0 para TikTok |
| Credenciais inválidas | App Key/Token expirados | Atualizar credenciais no DynamoDB |
...
Por que 20?
- Balanceamento entre throughput e timeout
- VTEX API suporta bem essa taxa
- Lambda tem tempo suficiente para processar

##### 2. DynamoDB vs VTEX API

```javascript
// ❌ Lento: buscar todos os SKUs da VTEX
const skuList = await getSkuList(); // 10.000+ SKUs, demora minutos

// ✅ Rápido: buscar apenas SKUs sincronizados com TikTok
const skuList = await getSkuListVtex(clientHash); // Apenas produtos ativos
```

##### 3. SQS para Desacoplamento

- **Busca de estoque** e **atualização no TikTok** são processos independentes
- SQS permite retry automático em caso de falha
- Escalabilidade: múltiplas Lambdas consomem a fila

#### Melhorias Futuras

- [ ] Implementar cache de estoque (Redis)
- [ ] Paralelizar busca de estoque (Promise.all)
- [ ] Suportar múltiplos warehouses VTEX
- [ ] Webhook VTEX para atualização em tempo real
- [ ] Dashboard de sincronização de estoque
- [ ] Alertas quando estoque < X unidades
- [ ] Histórico de mudanças de estoque

#### Segurança

:::warning Credenciais VTEX
As credenciais VTEX (App Key/Token) têm acesso **total** à conta. Devem ser:
- Armazenadas **criptografadas no DynamoDB**
