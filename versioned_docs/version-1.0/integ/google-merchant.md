...
description: Integração completa com Google Merchant Center - OAuth2, Promoções e Content API
keywords: [google merchant, content api, oauth2, shopping, promoções]
tags: [integrações, google, merchant, promoções]
---

# Integração com Google Merchant Center

Integração completa via Content API for Shopping v2.1, incluindo autenticação OAuth2, gestão de promoções e sincronização bilateral com Google Merchant Center.

:::tip Sincronização Bilateral
O módulo permite criar promoções localmente e sincronizá-las com o Google, ou importar promoções existentes do Google Merchant Center.
:::
...
- [OAuth2 e Autenticação](#oauth2-e-autenticação)
- [Módulo de Promoções](#módulo-de-promoções)
- [DynamoDB](#dynamodb)
...
- **API**: Google Shopping Content API v2.1
- **Autenticação**: OAuth 2.0 com tokens persistidos em DynamoDB
...
- **Armazenamento**: DynamoDB (`ssxml_promotions`, `ssxml_google_merchant_tokens`)
- **Biblioteca**: Cliente oficial Google API PHP
...
- **`PromotionController`**: CRUD de promoções, sincronização, filtros e exportação
- **`OAuth2GoogleController`**: Fluxo de autorização OAuth2 e gestão de tokens

### Services

- **`GoogleMerchantPromotionService`**: Sincronização e conversão de formatos (interno ↔ Google)
- **`GoogleMerchantTokenService`**: Gestão e renovação automática de tokens OAuth2
- **`PromotionService`**: Operações DynamoDB (CRUD, filtros, busca)

### Models

- **`PromotionForm`**: Formulário e validação de promoções
- **`PromotionLog`**: Logs de erro e eventos
...
sequenceDiagram
    ...
    User->>OAuth2Controller: GET /oauth2/google/authorize
    OAuth2Controller->>Google: Redirect para autorização
    Google->>User: Tela de consentimento
    User->>Google: Autoriza acesso
    Google->>OAuth2Controller: Callback com code
    OAuth2Controller->>Google: Troca code por tokens
    Google-->>OAuth2Controller: access_token + refresh_token
    OAuth2Controller->>TokenService: saveTokenData()
    TokenService->>DynamoDB: Persiste tokens
    OAuth2Controller-->>User: Redirect para dashboard
```
...
- `GET /oauth2/google/callback` - Processar callback do Google
- `POST /oauth2/google/revoke` - Revogar acesso

### Renovação automática de tokens

`GoogleMerchantTokenService::hasActiveToken()` verifica e renova tokens automaticamente:

- **Margem de segurança**: 10 minutos antes da expiração
- **Processo**: Usa `refresh_token` para obter novo `access_token`
- **Persistência**: Atualiza token no DynamoDB automaticamente
- **Fallback**: Tenta usar token atual se renovação falhar

**Código de exemplo:**

```php
// Verifica se há token ativo e renova se necessário
if (!$tokenService->hasActiveToken($clientHash, $merchantId)) {
    throw new \Exception('Token inválido. Cliente precisa reautorizar.');
}

$token = $tokenService->getActiveToken($clientHash, $merchantId);
```
...
Usado para importação de promoções existentes do Google (funcionalidade secundária).
...
| `404 Not Found` | Promoção não existe no Google | Normal para novas promoções |
| `400 Bad Request` | Validação falhou | Verificar payload e logs |
| `NoSuchKey` | Token não encontrado no DynamoDB | Cliente precisa autorizar |
...
- [DynamoDB](../infra/dynamodb.md) - Estrutura das tabelas
- [API Endpoints](../backend/api-endpoints.md) - Referência completa da API
- [Serviços](../arquitetura/servicos.md) - Services envolvidos
:::
